...
class FaceVerificationAnalyzer(
        ...
        if (mediaImage != null) {
            ...
            faceDetector.process(image)
                .addOnSuccessListener { faces ->
                    ...
                    val shadowRatio = calculateShadowRatio(finalBitmap, face)
                    if (shadowRatio < FaceVerificationConfig.SHADOW_RATIO_THRESHOLD) {
                        onStateChanged(LivenessState.TOO_MUCH_SHADOW)
                        imageProxy.close()
                        return@addOnSuccessListener
                    }
    ...
    companion object {
        fun calculateShadowRatio(fullBitmap: Bitmap, face: com.google.mlkit.vision.face.Face): Float {
            ...
            val leftCheek = face.getLandmark(com.google.mlkit.vision.face.FaceLandmark.LEFT_CHEEK)
            val rightCheek = face.getLandmark(com.google.mlkit.vision.face.FaceLandmark.RIGHT_CHEEK)

            if (leftEye == null || rightEye == null || leftCheek == null || rightCheek == null) {
                return 1.0f // Fallback if landmarks fail
            }

            // Dynamic patch size based on face width to handle different distances
            val faceWidth = face.boundingBox.width()
            val patchRadius = maxOf(2, (faceWidth * 0.05).toInt())

            val lEyeBright = getPatchBrightness(fullBitmap, leftEye.position.x.toInt(), leftEye.position.y.toInt(), patchRadius)
            val rEyeBright = getPatchBrightness(fullBitmap, rightEye.position.x.toInt(), rightEye.position.y.toInt(), patchRadius)
            val lCheekBright = getPatchBrightness(fullBitmap, leftCheek.position.x.toInt(), leftCheek.position.y.toInt(), patchRadius)
            val rCheekBright = getPatchBrightness(fullBitmap, rightCheek.position.x.toInt(), rightCheek.position.y.toInt(), patchRadius)

            val avgEyeBright = (lEyeBright + rEyeBright) / 2.0f
            val avgCheekBright = (lCheekBright + rCheekBright) / 2.0f

            return avgEyeBright / (avgCheekBright + 1e-6f)
        }

        private fun getPatchBrightness(bitmap: Bitmap, cx: Int, cy: Int, radius: Int): Float {
            val w = bitmap.width
            val h = bitmap.height

            val x1 = maxOf(0, cx - radius)
            val x2 = minOf(w, cx + radius)
            val y1 = maxOf(0, cy - radius)
            val y2 = minOf(h, cy + radius)

            val patchW = x2 - x1
            val patchH = y2 - y1

            if (patchW <= 0 || patchH <= 0) return 0.0f

            val pixels = IntArray(patchW * patchH)
            bitmap.getPixels(pixels, 0, patchW, x1, y1, patchW, patchH)
            ...
            for (color in pixels) {
                ...
                sumGray += (0.299 * r + 0.587 * g + 0.114 * b).toInt()
            }

            return sumGray / (pixels.size.toFloat())
        }
